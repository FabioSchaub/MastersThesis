"""Run the repair on every design CSV in ``pipeline/`` and summarise how many pairs it fixed.

For each design the script counts, before and after the repair, how many connected pairs
satisfy the two screwdriving criteria analytically and how many the surrogate calls feasible,
and prints the two counts side by side per design and as a total. Keeping both is the point:
the surrogate count is what the repair optimised against, the analytical count is whether it
was right, and a gap between them is a bluff.

Reads ``pipeline/*.csv`` other than repaired or output files, prints to the console and writes
nothing. The thresholds are imported from the repair rather than restated, so this can never
score against a different rule than the one the repair enforced.

Run:
    python tools/batch_test_repair.py                 # every design in pipeline/
    python tools/batch_test_repair.py table letter    # only names containing these
"""
...
def per_pair_metrics(blocks: dict, conns: list[tuple[int, int]],
    ...
    """Score every connected pair analytically and, if a model is given, with the surrogate too."""
    rows = []
    for i, j in conns:
        if i not in blocks or j not in blocks:
            continue
        b_i, b_j = blocks[i], blocks[j]
        # The lower block is the base and the upper one the part under test, matching the
        # convention the surrogate was trained under. Swapping them would invert the roles the
        # node indices encode.
...
def analyze_csv(csv_text: str, gnn_model, scale_factor: float,
    ...
    """Parse one design CSV and return its per-pair scores plus the feasible counts."""
...
def run_one(csv_path: Path, gnn_model, scale_factor: float,
            device: torch.device) -> dict:
    """Score one design, repair it, score it again; a failed repair is recorded, not raised."""
    ...
    try:
        ...
        ok = True
        err = None
    # One design that fails must not end the sweep; the error is carried in the result row and
    # printed with the rest so the remaining designs still get scored.
...
def main() -> int:
    ...
    # Skip the products of earlier runs, otherwise an already-repaired design would be counted
    # as an input and inflate the before-repair score.
    csvs = sorted(
    ...
    # Designs that raised are excluded from the totals rather than counted as zero, so a crash
    # is visible as a missing design instead of quietly depressing the rate.
