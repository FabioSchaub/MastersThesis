"""The repair of Part I, packaged to run on the robotic cell with nothing else installed.

Called from the interface as the first of the two entry points, under the CSV contract
described there: one design in, the same design repaired out, with the fasteners placed.

This file is a self-contained copy. The surrogate, the closed-form geometry, the gradient
repair and the screw placement are all restated here, and the only thing it needs beside it is
a checkpoint in the neighbouring ``fabio_model/`` directory. That duplication is deliberate:
the cell runs this without the repository around it, so nothing is imported from ``src/`` or
``pipeline/``, and the hyperparameters are constants at the top of the file rather than a
configuration that would have to be shipped alongside.

The consequence is that it can drift from the repository version. The behaviour it is meant to
match is the one in ``pipeline/repair_strategies.py`` and ``src/repair_optimizer.py``, and a
change to either belongs here too.
"""
...
class _GNN(nn.Module):
    """Late-fork GNN with raw size + node-type as 5d node features, 6d edge features.

    Inputs:
        node = [size_x, size_y, size_z, type_0, type_1]
        edge = [Δx, Δy, Δz, dist_xy, dist_xz, dist_yz]
...
def _load_model() -> dict:
    """Load the GNN + target stats from fabio_model/ (cached across calls)."""
    if _loaded:
        return _loaded

    candidates = sorted(_MODEL_DIR.glob("*.pth"), key=lambda p: p.stat().st_mtime)
    if not candidates:
        raise FileNotFoundError(
            f"No .pth in {_MODEL_DIR}. Drop the trained GNN checkpoint there."
        )
...
def _adam_repair_batched(
    ...
    """Pure-Adam batched repair on (size_active, pos_active).

    Returns dict with: size_active_out (N,3), pos_active_out (N,3),
    ...
    # Size box: upper bound symmetric; lower bound asymmetric (contact axis
    # may shrink to (1 - CONTACT_SHRINK_ALPHA), tangential cannot shrink).
    ...
    # Per-sample pos-freeze: probe GNN; for samples whose predicted overlap
    # already passes the threshold, zero out pos.grad for the entire run.
    ...
    for step in range(NUM_STEPS):
        ...
        batch = _build_batch(size_anchor_all, size_active, pos_anchor_all, pos_active)
        ...
        if (step + 1) % EARLY_STOP_INTERVAL == 0:
            with torch.no_grad():
                batch_es = _build_batch(
                    size_anchor_all, size_active, pos_anchor_all, pos_active
                )
    ...
    with torch.no_grad():
        batch_f = _build_batch(size_anchor_all, size_active, pos_anchor_all, pos_active)
    ...
    while current_layer:
        ...
        for parent in sorted(current_layer):
            for child in sorted(adj[parent]):
                ...
                with torch.no_grad():
                    batch_probe = _build_batch(
                        size_t[parent].unsqueeze(0),
                        size_t[child].unsqueeze(0),
...
def optimization(input_csv: str) -> str:
    ...
    gnn_model = _load_model()["model"].to(device)
    repaired = _gnn_repair_layered(snapped, conns, gnn_model, device)
