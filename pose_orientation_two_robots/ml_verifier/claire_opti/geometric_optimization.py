"""The second repair model behind the interface, developed alongside this thesis.

Called from the interface as the second of the two entry points, under the same CSV contract:
one design in, the same design repaired out. Like the repair of Part I it works joint by joint
and descends the gradient of a learned feasibility model, but it uses its own surrogate and
its own scaling, both loaded from ``claire_model/`` next to this file.

Included here so that the interface is complete and the two repairs can be run on the same
design. It is not part of Part I and is not documented further.
"""
