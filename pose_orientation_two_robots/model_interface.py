"""Global ML refinement interface.

Two ML models run in parallel on the Gemini-winner YAML. Each takes a
flat single-row CSV with the whole scene's geometry and returns a CSV
of refined positions / sizes / screws. Joint connectivity (which block
is wired to which) is part of the input but the model is expected to
preserve it unchanged.

For each YAML containing N blocks the column layout is:

  Per block N (6 cols each):
    BlockN_PosX, BlockN_PosY, BlockN_PosZ     -- RViz frame, metres
    BlockN_SizeX, BlockN_SizeY, BlockN_SizeZ  -- world-AABB extents, metres

  Adjacency (upper triangle, N*(N-1)/2 cols):
    BlockI_ConnectsTo_BlockJ                  -- 0 or 1

  Per block N, N screw slots (3 cols each = 3N cols per block):
    BlockN_Screw0_X, BlockN_Screw0_Y, BlockN_Screw0_Z
    BlockN_Screw1_X, BlockN_Screw1_Y, BlockN_Screw1_Z
    ...
    BlockN_ScrewN-1_X/Y/Z
    NaN padded for unused slots.

Frame conventions:
  * Positions: metres, RViz frame (relative to TABLE_CENTER_X=0.4,
    TABLE_CENTER_Y=0.6, TABLE_SURFACE_HEIGHT=0.842 in world).
  * Sizes: metres, world-axis-aligned bounding-box extents (the part's
    orientation is already baked in -- when the long axis points up
    SizeZ becomes the long dimension). The model never returns a new
    orientation; orientation stays exactly what the winner YAML had.

The two ML-model entry points are clearly marked below
(``refine_full_yaml_model_a`` / ``refine_full_yaml_model_b``).
Currently both call a random-jitter stub with a fixed per-model seed so
the pipeline can be exercised end-to-end without real ML. Each colleague
replaces the body of THEIR function with the real model call. The
input/output CSV contract above does not change.
"""
