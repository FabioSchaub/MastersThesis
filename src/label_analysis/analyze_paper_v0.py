"""Measure how well the trained surrogate predicts the two criteria, and how well it decides.

Reads a surrogate checkpoint and one split of the dataset and reports both sides of the model:
the regression of overlap and thickness, and the feasibility decision. The regression is
summarised overall and again inside bands around each threshold, because a mean error over the
whole range says little about the region where a pair is close to passing and the decision is
actually made. The decision is evaluated twice, once as derived from the two regressed criteria
and once from the feasibility head, since the repair reads both.

This is the reproducible source of the surrogate metrics quoted for Part II: it recomputes them
from a checkpoint rather than reading them off a training curve. Scatter and residual plots go
to ``results/paper_v0_analysis/``, everything else is printed.

Run as ``python src/label_analysis/analyze_paper_v0.py``. ``GNN_MODEL`` selects a checkpoint in
the surrogate folder, ``GNN_SPLIT`` chooses ``train``, ``val`` or ``test``, and ``GNN_QUICK_N``
truncates the dataset for a fast pass.
"""
...
# The bands are narrow around each threshold and wide away from it, because that is where the
# error changes the verdict. A pair five millimetres past the limit is decided correctly
# whatever the error; one a millimetre short is not.
OVERLAP_BUCKETS = [
...
def find_paper_v0_checkpoint() -> Path:
    """Return the most recently written surrogate checkpoint, or exit if there is none."""
    candidates = sorted(
...
def load_model(ckpt_path: Path, target_stats: dict, device: torch.device) -> GNN:
    """Load a surrogate checkpoint, taking the target standardisation from the training split
    rather than from the checkpoint, so predictions are read on the scale they were fitted on."""
    model = GNN(
...
def predict(model: GNN, dataloader: DataLoader, device: torch.device):
    """Run the surrogate over a whole split and collect predictions and labels.

    Returns:
        Five arrays: predicted and true criteria, both ``(N, 2)`` in metres and overlap first;
        the feasibility probability ``(N,)``; the failure mode probabilities ``(N, 2)``; and
        the feasibility label ``(N,)``.
    """
    # Must match src/gnn_dataset_preparation.OVERLAP_LOG_EPS, otherwise the inverse below is
    # biased and every reported overlap error is off by a constant.
    ...
    # A negative mean can only come from a logarithm of a length, so its sign identifies which
    # convention the targets were standardised under.
    log_space_overlap = bool(target_mean[0].item() < 0.0)
    with torch.no_grad():
        for batch in dataloader:
            batch = batch.to(device)
            # Accepts a fourth output so that a checkpoint from a model with an extra head can
            # still be analysed on the two criteria it shares with this one.
...
def overall_stats(pred: np.ndarray, target: np.ndarray) -> dict:
    """Summarise one target over a whole split.

    Args:
        pred, target: ``(N,)`` each, in metres.

    Returns:
        A dictionary of scalars. Errors are reported in millimetres, which is the unit the two
        criteria are stated in. Besides the usual error measures it carries ``slope`` and
        ``std_ratio``: a model that regresses towards the mean of the targets has a small error
        and a slope well below one, and the error alone would not show it.
    """
    err = pred - target
    abs_err = np.abs(err)
...
def stratified_bucket_stats(pred: np.ndarray, target: np.ndarray, buckets: list):
    """Repeat the error statistics inside each band of true values, as a list of one row per
    band. Bands are half-open, ``[lo, hi)``, with bounds in metres."""
    rows = []
    for lo, hi, tag in buckets:
        m = (target >= lo) & (target < hi)
        n = int(m.sum())
        # Below fifty samples a slope is dominated by noise; the band is reported empty rather
        # than with a number that would invite reading.
        if n < 50:
...
def classifier_stats(true: np.ndarray, pred: np.ndarray) -> dict:
    """Score a binary decision, returning the usual rates and the four cells of the confusion
    matrix. The positive class is the feasible one."""
...
def plot_calibration(pred, target, name, out_path, threshold):
    """Scatter predicted against true values in millimetres, with the diagonal and the
    threshold drawn in."""
    target_mm = target * 1000
    pred_mm = pred * 1000
    fig, ax = plt.subplots(figsize=(6, 6))
    if len(target) > 10000:
        # Subsampled with a fixed seed so the figure is reproducible, and because a scatter of
        # the full split is an opaque block rather than a readable plot.
...
def plot_residuals(pred, target, name, out_path):
    """Histogram of prediction minus truth in millimetres, with zero and the mean marked."""
...
def main():
    # Seeded before the graphs are split, so that the split reproduced here is the one the
    # surrogate was trained and validated on and the test set is genuinely unseen.
    ...
    # --- 4-way fail-class confusion (NONE / OVERLAP / THICK / BOTH) ---
    # The mode heads are independent sigmoids, so BOTH = both heads > 0.5.
    # True label: NONE | OVERLAP | THICK | BOTH derived from reg_target.
    ...
    def class_id(ov, th):
        # 0=NONE, 1=OVERLAP only, 2=THICK only, 3=BOTH
