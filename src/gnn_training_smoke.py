"""Run the training path end to end on a small subset, without a cluster and without logging.

Uses the same dataset builder, the same objective and the same evaluation as the real run, so
a change that breaks any of them shows up here in a minute rather than in a queued job. It
writes no checkpoint and reports nothing to Weights and Biases: the point is that it finishes,
and that the error and the spread ratio printed at the end are not absurd.
...
Prints per-epoch metrics and a stratified breakdown of the last epoch to the console.
"""
...
def main() -> None:
    ...
    # Set before the dataset is built, so the subset is taken while the graphs are being made
    # rather than afterwards; building the full set would be the slowest part of this script.
